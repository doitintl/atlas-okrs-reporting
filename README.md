...
The script `tools/post_okr_comments.py` allows you to automatically post comments to malformed OKRs in Atlassian, based on the health checks performed on your OKR data. It supports three data sources:

### Data Sources Supported

1. **Local CSV file** (`--file`):
   - Use a CSV file generated by your local scraping or processing pipeline.
   - Example:
     ```bash
     python tools/post_okr_comments.py --file data/okrs.csv
     ```

2. **Cloud Storage bucket** (`--cloud`):
   - Automatically downloads the latest processed CSV from your configured GCS bucket.
   - Example:
     ```bash
     python tools/post_okr_comments.py --cloud
     ```

3. **BigQuery external table** (`--bigquery`):
   - Reads malformed OKRs directly from the BigQuery external table/view, using the same health logic as the Python scripts.
   - Example:
     ```bash
     python tools/post_okr_comments.py --bigquery
     ```

**Note:** Only one data source can be used at a time. The script will validate your arguments.

### How it works
- For each malformed OKR, the script generates a personalized English message explaining what is missing.
- It shows a preview of the message and the OKR's URL, and asks for confirmation before posting.
- If confirmed, it posts the comment to Atlassian using the API and credentials from `config.env`.
- The script uses the `EntityId` (Atlassian ARI) column to target the correct OKR in the API call. This column is now present in all data pipelines (CSV, Cloud, BigQuery).

### Example message
```
This OKR is missing some required fields: Progress Metric.
Please update this OKR ASAP to ensure it is healthy and trackable.
If you have any questions, please reach out to your line-up manager.
```

### Requirements
- The `EntityId` column must be present in your data (added by the latest scraping and Cloud Run jobs).
- All sensitive API credentials are loaded from `config.env`.
- The script is robust to both legacy and new data formats.

### Why EntityId?
- The Atlassian API requires the unique ARI (EntityId) to post comments to the correct OKR.
- The pipeline ensures this field is available in all data sources, enabling seamless automation.
